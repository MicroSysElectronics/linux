...
static int fxl6408_gpio_direction_input(struct gpio_chip *gc, unsigned off)
{
    ...
    i2c_smbus_write_byte_data(chip->client, FXL6408_IO_DIR, chip->reg_io_dir);
...
static void fxl6408_gpio_set_value(struct gpio_chip *gc, unsigned off, int val)
{
    ...
    i2c_smbus_write_byte_data(chip->client, FXL6408_OUTPUT, chip->reg_output);
    mutex_unlock(&chip->i2c_lock);
}
...
static void fxl6408_set_mask_worker(struct work_struct *work)
{
    ...
    mutex_lock(&chip->i2c_lock);
    i2c_smbus_write_byte_data(chip->client, FXL6408_INTERRUPT_MASK, chip->irq_mask);
    mutex_unlock(&chip->i2c_lock);
}
...
{
    return (type == IRQ_TYPE_LEVEL_LOW)
...
static void fxl6408_worker(struct work_struct *work)
{
    ...
    irq_status = i2c_smbus_read_byte_data(chip->client, FXL6408_INTERRUPT_STAT);
    mutex_unlock(&chip->i2c_lock);
    ...
    for (i = 0; i < chip->gpio_chip.ngpio; i++) {
        if ((irq_mask & BIT(i))==0
                && (irq_status & BIT(i))) {
            handle_nested_irq(irq_find_mapping(chip->gpio_chip.irq.domain, i));
        }
    }
...
//        snprintf(chip->irq_chip.name, "fxl6408-%d:%02x",
//                client->adapter->nr,
//                client->addr);
        ...
        chip->irq_chip.irq_bus_sync_unlock = fxl6408_irq_bus_sync_unlock;
                         ...
                         const struct i2c_device_id *id)
{
    ...
    if (of_property_read_u8_array(np, "pull-config", pull_cfg, 2)==0) {
        ...
        i2c_smbus_write_byte_data(client, FXL6408_PULL_ENABLE, pull_cfg[0]);
    }
    ...
    if (of_property_read_u8(np, "input-default-state", &state)==0) {
        i2c_smbus_write_byte_data(client, FXL6408_INPUT_DEFAULT_STATE, state);
    }
