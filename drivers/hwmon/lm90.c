...
static int lm90_set_limit(struct lm90_data *data, int index,
		enum lm90_limits limit, u32 val)
{
	int temp;
...
static int lm90_init_client(struct i2c_client *client, struct lm90_data *data)
{
	...
	if (data->kind == adt7461
		...
		if (config & 0x04) {
			...
			dev_info(&client->dev,
					"Extended Temperature Mode is ON\n");
		}
		else {
			config |= 0x04;
			data->flags |= LM90_FLAG_ADT7461_EXT;
			dev_info(&client->dev,
					"Extended Temperature Mode set OK\n");
		}
	}
	else {
	...
	if (cnt > 0) {
		if (of_property_read_u32(client->dev.of_node,
				"#limit-cells", &limit_cells))
			// default: <min max crit hyst>
			limit_cells = limits_count;
		if (limit_cells > 0) {
			cnt /= limit_cells;
			if (limit_cells > limits_count)
				limit_cells = limits_count;
			for (i = 0; i < cnt; i++) {
