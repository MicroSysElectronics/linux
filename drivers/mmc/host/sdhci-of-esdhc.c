...
static void esdhc_of_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (clock_fixup == 0 || clock < clock_fixup)
		clock_fixup = clock;
	/*
	 * Limit SD clock to 20MHz for ls1043a according to its datasheet,
	 * because of the level shifter it does not work above 20MHz with some carriers.
	 */
	if (clock > 20000000 &&
	    of_find_compatible_node(NULL, NULL, "fsl,ls1043a-esdhc"))
		clock = 20000000;
...
static void esdhc_init(struct platform_device *pdev, struct sdhci_host *host)
{
	...
	if (np) {
		...
		if (!IS_ERR(clk)) {
			 ...
			 */
			if (of_device_is_compatible(np, "microsys,mpxls1046-esdhc")) {
				esdhc->peripheral_clock = clk_get_rate(clk);
			}
			else if (of_device_is_compatible(np, "fsl,ls1046a-esdhc") ||
