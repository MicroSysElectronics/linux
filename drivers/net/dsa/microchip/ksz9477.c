...
static int ksz9477_switch_detect(struct ksz_device *dev)
{
	...
	/*
	 * First guess is to assume the total number of ports
	 * that the switch has. This can be corrected later in
	 * the initialization process.
	 */
	dev->mib_port_cnt = dev->port_cnt;
...
static int ksz9477_switch_init(struct ksz_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(ksz9477_switch_chips); i++) {
		...
		if (dev->chip_id == chip->chip_id) {
			...
			dev->mib_port_cnt = chip->port_cnt;
	...
	/*
	 * Initialize the bitmask of all user ports with 0.
	...
	/*
	 * Allocate the array of ports. Assume the maximum of ports that the
	 * switch provides. We can never know what ports are configured in
	 * the device tree.
	 */
	dev->ports = devm_kzalloc(dev->dev, sizeof(struct ksz_port) * dev->port_cnt,
				  GFP_KERNEL);
	if (!dev->ports)
		return -ENOMEM;
	for (i = 0; i < dev->mib_port_cnt; i++) {
