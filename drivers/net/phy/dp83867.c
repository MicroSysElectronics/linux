...
static int dp83867_of_init(struct phy_device *phydev)
{
	...
	dp83867->loopback = LOOPBACK_NONE;

	if (of_property_read_bool(of_node, "mii-loopback")) {
		dp83867->loopback = LOOPBACK_MII;
	}
	else if (of_property_read_bool(of_node, "ti,pcs-loopback")) {
		dp83867->loopback = LOOPBACK_PCS;
	}
	else if (of_property_read_bool(of_node, "ti,digital-loopback")) {
		dp83867->loopback = LOOPBACK_DIGITAL;
	}
	else if (of_property_read_bool(of_node, "ti,analog-loopback")) {
		dp83867->loopback = LOOPBACK_ANALOG;
	}
	else if (of_property_read_bool(of_node, "ti,reverse-loopback")) {
		dp83867->loopback = LOOPBACK_REVERSE;
	}

	if (of_property_read_bool(of_node, "ti,sgmii-an-disable"))
...
static int dp83867_config_loopback(struct phy_device *phydev)
{
	struct dp83867_private *const dp83867 = phydev->priv;
	int val;

	if (dp83867->loopback == LOOPBACK_MII) {
		...
		phy_write(phydev, DP83867_CTRL,     0x8000);
		phy_write(phydev, MII_BMCR,         0x0140);
		if (phy_interface_is_rgmii(phydev))
			phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_RGMIICTL, 0x00d3);
		phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_LOOPCR,   0xe720);
		switch (dp83867->loopback) {
		case LOOPBACK_PCS:
			phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_BISCR,    0x0001);
			break;
		case LOOPBACK_DIGITAL:
			phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_BISCR,    0x0004);
			break;
		case LOOPBACK_ANALOG:
			phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_BISCR,    0x0008);
			break;
		case LOOPBACK_REVERSE:
			phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_BISCR,    0x0020);
			break;
		default:
			break;
		}
		phy_write(phydev, DP83867_CTRL, 0x4000);
	}
...
static int dp83867_config_init(struct phy_device *phydev)
{
	...
	if (phydev->interface == PHY_INTERFACE_MODE_SGMII) {
		...
		if (dp83867->sgmii_an_enabled)
			ret = phy_modify(phydev, DP83867_CFG2,
	...
	if (dp83867->loopback != LOOPBACK_NONE) {
		dp83867_config_loopback(phydev);
	}
