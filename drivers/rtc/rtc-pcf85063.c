...
static void pcf85063_worker(struct work_struct *work)
{
    struct pcf85063 *pcf85063;
    int reg, err;
    unsigned long events = 0;

    pcf85063 = container_of(work, struct pcf85063, irq_work);

    err = regmap_read(pcf85063->regmap, PCF85063_REG_CTRL2, &reg);
    if (err) {
        dev_err(&pcf85063->rtc->dev, "failed to read IRQ status!\n");
        return;
    }

    if (reg >= 0) {
        if ((reg & 0xc0) == 0xc0) { // AIE and AF
            events |= RTC_IRQF | RTC_AF;
            regmap_write(pcf85063->regmap, PCF85063_REG_CTRL2, reg&~BIT(6)); // clear AF
        }
        rtc_update_irq(pcf85063->rtc, 1, events);
    }

    enable_irq(pcf85063->irq);
}
...
static irqreturn_t pcf85063_rtc_handle_irq(int irq, void *dev_id)
{
    ...
    disable_irq_nosync(pcf85063->irq);

    schedule_work(&pcf85063->irq_work);
...
static int pcf85063_probe(struct i2c_client *client)
{
    ...
    if (ret > 0) pcf85063->irq = ret;
    else pcf85063->irq = -1;

    if (pcf85063->irq > 0) {

        err = regmap_read(pcf85063->regmap, PCF85063_REG_CTRL1, &tmp);
        if (err >= 0 && (tmp&BIT(2))!=0) {
            regmap_write(pcf85063->regmap, PCF85063_REG_CTRL1, tmp&~BIT(2));
        }

        err = regmap_read(pcf85063->regmap, PCF85063_REG_CTRL2, &tmp);
        if (err >= 0 && ((tmp&~0x7)!=0)) {
            regmap_write(pcf85063->regmap, PCF85063_REG_CTRL2, tmp&0x7);
        }

			pcf85063->rtc->ops = &pcf85063_rtc_ops_alarm;
			device_init_wakeup(&client->dev, true);

        INIT_WORK(&(pcf85063->irq_work), pcf85063_worker);
        err = devm_request_threaded_irq(&client->dev, pcf85063->irq,
                NULL,
                ...
                "pcf85063", pcf85063);
        if (err < 0) {
            dev_err(&client->dev, "unable to register PCF85063 RTC interrupt\n");
            return err;
        }

        err = dev_pm_set_wake_irq(&client->dev, pcf85063->irq);
			if (err)
